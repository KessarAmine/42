...
/*   Updated: 2023/03/18 14:58:43 by kmohamed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*==Config Folder*/
int							destination_valid(int player_x, int player_y,
								t_env *env);
int							can_move(int p_x, int p_y, t_directions d,
								t_env *env);
...
int							init(t_env *env, char **argv,
								t_map_constraints *map_constraints);
void						ft_put_element(int x, int y, t_env *env,
								t_elements element);
...
void						sub_(t_map_constraints *map_c, t_env *e, int *i,
								size_t *j);
void						sub_sub(t_map_constraints *map_c, t_env *env,
								int *i, size_t *j);
...
void						ft_putnbr_hexlower_fd(unsigned long long nbr,
								int fd, int *count);
void						ft_putnbr_hexupper_fd(unsigned long long nbr,
								int fd, int *count);
...
void						get_conversion(va_list args, char signe,
								int *count);
void						mark(t_map_constraints *map_c, t_env *env, int *i,
								size_t *j);
int							exit_game(t_env *env);
/*so_long*/
int							path(t_env *env);
void						path_parsing(char **clone, t_env *env, int i,
								size_t j);
