...
/*   Updated: 2023/03/18 15:34:50 by kmohamed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_img(t_env *env)
{
	...
	env->floor = mlx_xpm_file_to_image(env->mlx, env->path_floor, &i, &j);
	env->path_wall = "assets/textures/wall.xpm";
	env->wall = mlx_xpm_file_to_image(env->mlx, "assets/textures/wall.xpm", &i,
			&j);
	env->path_baby = "assets/characters/baby.xpm";
	env->baby = mlx_xpm_file_to_image(env->mlx, env->path_baby, &i, &j);
	env->path_pappy = "assets/characters/pappy.xpm";
	env->pappy = mlx_xpm_file_to_image(env->mlx, env->path_pappy, &i, &j);
	env->path_pie = "assets/collectables/pie.xpm";
	env->pie = mlx_xpm_file_to_image(env->mlx, env->path_pie, &i, &j);
	env->path_bed = "assets/exit/bed.xpm";
	env->bed = mlx_xpm_file_to_image(env->mlx, env->path_bed, &i, &j);
}
...
{
	if (element == FLOOR)
		mlx_put_image_to_window(env->mlx, env->window, env->floor, x * 32, y
			* 32);
	else if (element == WALL)
		mlx_put_image_to_window(env->mlx, env->window, env->wall, x * 32, y
			* 32);
	else if (element == BED)
		mlx_put_image_to_window(env->mlx, env->window, env->bed, x * 32, y
			* 32);
	else if (element == PIE)
		mlx_put_image_to_window(env->mlx, env->window, env->pie, x * 32, y
			* 32);
	else if (element == BABY)
		mlx_put_image_to_window(env->mlx, env->window, env->baby, x * 32, y
			* 32);
	else if (element == DADDY)
		mlx_put_image_to_window(env->mlx, env->window, env->pappy, x * 32, y
			* 32);
}
